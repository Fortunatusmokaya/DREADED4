...
export default class Command extends BaseCommand {
    ...
    run = async (M: ISimplifiedMessage): Promise<void> => {
        ...
        M.mentioned.forEach(async (user) => {
            ...
            if (M.groupMetadata?.admins?.includes(user)) M.reply(`✖Sir don't be a fool, I have Skipped *${username}* as they're already an admin`)
            else {
                await this.client.groupMakeAdmin(M.from, [user])
                M.reply(`👑Sir I have Successfully Promoted *${username}*`)
            }
        })
    }
}
