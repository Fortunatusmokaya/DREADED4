...
export default class Command extends BaseCommand {
    ...
    run = async (M: ISimplifiedMessage): Promise<void> => {
        ...
        M.mentioned.forEach(async (user) => {
            ...
            if (!M.groupMetadata?.admins?.includes(user)) M.reply(`✖Sir you on drugs? I have Skipped *${username}* as they're not an admin`)
            else if (user !== this.client.user.jid) {
                await this.client.groupDemoteAdmin(M.from, [user])
                M.reply(`➰Sir I have Successfully Demoted *${username}*`)
            }
        })
    }
}
